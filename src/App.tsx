...
import BellView from '../src/components/BellView.jsx';
import DaysCarousel from '../src/components/DaysCarousel.jsx';
import StarButtonView from '../src/components/StarButtonView.jsx'
...
export class App extends React.Component<IAppProps, IAppState> {
  ...
  getStartFirstLesson(todayOrTomorrow: string): string | undefined {
    ...
    for (let bell in this.state.days[this.state.today - day]) {
      if (this.state.days[this.state.today - day][bell][0].startAndfinishTime !== "") {
        return this.state.days[this.state.today - day][bell][0].startAndfinishTime.slice(0, 5)
      }
    }
  }
  ...
  whatLesson(date: Date, when: string): { lesson: string | undefined, type: string, num: number | undefined} { //определяет название пары, которая идет или будет
          ...
          for (let bell in this.state.days[this.state.today - 1]) {
            if ((this.state.days[this.state.today - 1][bell][0][5][0] === this.getCurrentLesson(date)) && (this.state.days[this.state.today - 1][bell][0][5][0] !== "")) {
              return {
                ...
                type: "now",
                num : parseInt(this.getCurrentLesson(date))
              };
            }
          }
          ...
          for (let bell in this.state.days[this.state.today - 1]) {
            ...
            if ((this.state.days[this.state.today - 1][bell][0][5][0] == parseInt(this.getCurrentLesson(date)) + 1) && (this.state.days[this.state.today - 1][bell][0][5][0] !== "")) {
              return {
                lesson: this.state.days[this.state.today - 1][bell][0][0],
                type: "next",
                num: parseInt(this.getCurrentLesson(date)) + 1
              };
            }
          }
...
*/}
  ...
  isCorrect() {
    ...
    if ((this.state.correct === true) && (correct_sub === true) && (correct_eng === true)) {
      ...
      this.setState({ flag: true });
      this.convertIdInGroupName();
      this.setState({ page: 7, color_group: "Предупреждение" });
    } else if (this.state.correct === true) {
      this.setState({color_group: "Предупреждение" });
    } else if (this.state.group === "") {
      this.setState({  color_group: "error" })
    } else {
      this.setState({ color_group: "error" })
    }
  ...
  render() {
    ...
    switch (this.state.page) {
      ...
      case 16:
        return <Dashboard state={this.state} setValue={this.setValue} getCurrentLesson={this.getCurrentLesson} whatLesson={this.whatLesson}
        ></Dashboard>
      default:
        break;
    }
